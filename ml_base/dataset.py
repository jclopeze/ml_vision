...
class IDataset(abc.ABC):
    ...
    @property
    def train(self) -> IDataset:
        return self.copy().filter_by_partition(Partitions.TRAIN, drop_field=True, inplace=True)

    @property
    def test(self) -> IDataset:
        return self.copy().filter_by_partition(Partitions.TEST, drop_field=True, inplace=True)

    @property
    def val(self) -> IDataset:
        return self.copy().filter_by_partition(
            Partitions.VALIDATION, drop_field=True, inplace=True)
    ...
    @classmethod
    @abc.abstractmethod
    def from_csv(cls, source_path: str) -> IDataset:
        pass

    @classmethod
    @abc.abstractmethod
    def from_datasets(cls, *args: IDataset, **kwargs) -> IDataset:
        pass
    ...
    #   region ACCESSORS
    def as_dataframe(self: IDataset, columns: List[str] = None) -> pd.DataFrame:
    ...
    def get_chunk(self, num_chunks: int, chunk_num: int) -> IDataset:
    ...
    def set_field_values(self,
                         ...
                         inplace: bool = True) -> IDataset:
        pass

    def split(self: IDataset,
              train_perc: float,
              *,
              test_perc: float = None,
              val_perc: float = None,
              group_by_field: str = None,
              stratify: bool = True,
              random_state: int = 42) -> IDataset:
        ...
        if self.is_empty:
            return self
        test_perc = 1 - train_perc if test_perc is None else train_perc
        val_perc = 0 if val_perc is None else val_perc
        ...
        if not group_by_field is None and group_by_field in df:
            total = 0.
            ...
            for part, perc in ordered_partitions.items():
                ...
                if np.isclose(total + perc, 1.):
                    ...
                    strat_labels = df.loc[ordinals][Fields.LABEL].values if stratify else None
                    part_size = perc / (1. - total)
    ...
    def filter_by_field(self,
        ...
        if mode == 'include':
            df = df[df[field].isin(values)].reset_index(drop=True)
        else:
            df = df[~df[field].isin(values)].reset_index(drop=True)
    ...
    def filter_by_partition(self,
        ...
        if not partition is None:
            ...
            return self.filter_by_field(
                Fields.PARTITION, partition, mode=mode, drop_field=drop_field, inplace=inplace)
        return self
    ...
    def take(self, n: int, random_state: int = None) -> IDataset:
        return self.sample(
    ...
    @classmethod
    @abc.abstractmethod
    def cast(cls, dataset: IDataset) -> IDataset:
        pass
    ...
    def _get_categories(self, sort_by_name: bool = True) -> list:
        """Get the list of distinct values in the `Fields.LABEL` field of the dataset.
    ...
    @classmethod
    @abc.abstractmethod
    def _copy_dataset(cls, dataset: IDataset) -> IDataset:
        pass
...
"""
    ...
    class AnnotationFields(IDataset.AnnotationFields):
        ITEM: Final = Fields.ITEM
    ...
    @property
    def data(self) -> pd.DataFrame:
        return self._merge_annotations_and_metadata()
    ...
    def from_csv(cls,
        ...
        **kwargs :
        ...
        df = cls._get_dataframe_from_csv(source_path=source_path, usecols=usecols)
    ...
    # TODO: Improve this method and unify it for different types of datasets
    def set_field_values(self,
                         field: str,
                         values: Union[Callable, pd.DataFrame, object],
        ...
        if isinstance(values, pd.DataFrame):
            ...
            if callable(values):
                ...
                if is_array_like(values):
                    assert_msg = "The size of the values does not match the size of the dataset."
                    assert len(df) == len(values), assert_msg
                df[field] = values
    ...
    def _filesystem_writer(self,
        ...
        if preserve_directory_hierarchy and not root_dir is None:
    ...
    def set_root_dir(self,
        ...
        """
        assert not root_dir is None or validate_filenames is False, "root_dir must be assigned"

        if not root_dir is None or validate_filenames is True:
    ...
    def _download(cls,
        ...
        **kwargs :
        ...
        """
        cond = not media_base_url is None
    ...
    def _apply_filters_and_mappings(self,
        ...
        if not exclude_categories is None:
            self.filter_by_categories(exclude_categories, mode='exclude', inplace=True)
        if not min_score is None or not max_score is None:
            self.filter_by_score(min_score=min_score, max_score=max_score, inplace=True)
        if not round_score_digits is None and Fields.SCORE in self.fields:
    ...
    def _get_filename(cls,
        ...
        """
        if not set_filename_with_id_and_extension is None:
